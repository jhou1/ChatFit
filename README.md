...
- 审批回复由配置的 LM 通过结构化输出按语义分类；“是的”“确认保存”“当然，就这样保存吧”等不包含数据修改的肯定回复会执行已展示的待保存内容。对 “OK”“好的”“确认” 这类独立的常见肯定词，系统会使用高置信度快速通道，避免被误判为闲聊；其他回复仍会连同待保存草稿发送给配置的 LM 分类。
- 如果确认回复同时补充或修改了训练信息，ChatFit 不会立即写入；它会更新待保存内容并再次请求确认。确认后的重复投递使用幂等键，不会生成重复训练记录。
- 待确认请求带有创建时间戳，超过 5 分钟（`HITL_TIMEOUT_SECONDS`）未被回复即过期；过期的待写入会被终止取消，不会执行写入。
- 待确认期间收到与审批无关的新消息（例如新训练或饮食描述）时，配置的 LM 会先分类该消息：确认是无关的新请求后，取消待写入并将新消息作为全新输入重新路由，不会再被上一次确认吞掉。
