...
async def generate_conversational_approval(tool_calls: list, llm_config: LLMConfig) -> str:
...
async def _classify_approval_intent(user_message: str, llm_config: LLMConfig) -> tuple[bool, str]:
    llm = create_chat_model(llm_config)
    prompt = f"""
...
async def chat_endpoint(req: ChatRequest, request: Request):
    ...
    if interrupts:
        ...
        is_approved, feedback = await _classify_approval_intent(req.message, request.app.state.llm_config)
        resume_data = {intr.id: {"approved": is_approved, "feedback": feedback} for intr in interrupts}
    ...
    async for event in request.app.state.agent.astream(action_command, config=config, stream_mode="updates"):
        # HITL interruptions
        if "__interrupt__" in event:
            interruption_data = event["__interrupt__"][0].value
            tool_calls = interruption_data.get("tool_calls", [])
            reply = await generate_conversational_approval(tool_calls, request.app.state.llm_config)
