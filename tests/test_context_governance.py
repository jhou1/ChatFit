...
async def test_context_governance_truncates_messages(mock_route, mock_execute, temp_db_path):
    # Setup mock returns
    mock_route.return_value = ["chatter"]
    
    mock_execute.return_value = {"messages": AIMessage(content="This is a new summary.")}
...
@patch("agents.roles.supervisor._execute_llm_query_safely", new_callable=AsyncMock)
@patch("agents.roles.supervisor.route_assistant_on_relevance", new_callable=AsyncMock)
async def test_context_governance_preserves_tool_calls(mock_route, mock_execute, temp_db_path):
    mock_route.return_value = ["chatter"]
