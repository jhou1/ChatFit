...
- [x] Long term memory
- [x] compacting memory on condition
...
- [x] Tool should not try to record when the practice type is with load
- [x] add tool to retrospect training history using time range provided by user
...
- [x] long term memory using store
...
- [ ] Create a handler to handle OpenAI content and Gemini content so I could reuse it in my agents

# Exception handling
- [x] Add retry mechanism for transient and rate limit errors, each retry
...
# Configurable
- [ ] Make max retries, etc configurable
