    ...
    for case in cases:
        ...
        for turn_idx, turn in enumerate(case.get("turns", [])):
            ...
            for t in turn.get("expected_tools", []):
                assertion = {"eval_type": "tool_call", "expected_tool": t["name"]}
            ...
            for r in turn.get("expected_routes", []):
                expected_trajectory_eval.append(
                    {"eval_type": "routing", "expected_agent": r}
                )
            ...
            response_eval = {}
            if (
                "expected_response_contains" in turn
                and turn["expected_response_contains"]
            ):
                response_eval["must_contain_semantics"] = turn[
