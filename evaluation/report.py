...
class ExperimentReport(BaseModel):
    ...
    def metrics(self) -> dict[str, Any]:
        ...
        tool_cases = [
            c
            for c in self.cases
            if "Tool Calling" in c.tags or "Multi-Agent Collaboration" in c.tags
        ]
        tool_passed = sum(
            1
            for c in tool_cases
            if not any(
                fc in ["missing_tool", "tool_params", "tool_args", "avoid_tool"]
        ...
        context_cases = [
            c
            for c in self.cases
            if "Memory & Cross-turn" in c.tags or "Multi-turn Dialogue" in c.tags
        ]
        context_consistency = (
            sum(c.passed for c in context_cases) / len(context_cases)
            if context_cases
            else 1.0
        )
        ...
        recovery_cases = [
            c
            for c in self.cases
            if "Intent Routing" in c.tags
            or any(fc == "clarification_failed" for fc in c.failure_codes)
        ]
        recovery_rate = (
            sum(c.passed for c in recovery_cases) / len(recovery_cases)
            if recovery_cases
            else 1.0
        )
        ...
        # LLM Scores
        overall_llm_scores = [
            c.overall_llm_score for c in self.cases if c.overall_llm_score is not None
        ]
        ...
        return {
            ...
            "average_llm_score": (
                sum(overall_llm_scores) / len(overall_llm_scores)
                if overall_llm_scores
                else None
            ),
    ...
    def release_gate(self, thresholds: ReleaseThresholds | None = None) -> ReleaseGate:
        ...
        if configured.require_llm_scores:
            ...
            if p10_llm is not None and p10_llm < configured.minimum_p10_llm_score:
                failures.append(
                    f"打分 P10 分位数 {p10_llm:.3f} < "
                    f"{configured.minimum_p10_llm_score:.3f}"
                )
    ...
    def to_markdown(self, thresholds: ReleaseThresholds | None = None) -> str:
        ...
        if not dim_summary:
            ...
            for dim_name in sorted(dim_summary.keys()):
                ...
                lines.append(
                    f"| {dim_name} | {d['count']} | {d['avg_weight']:.2f} | **{d['avg_score']:.2f}** / 5.0 |"
                )

        overall_score = metrics["average_llm_score"]
        overall_str = (
            f"**{overall_score:.2f}**" if overall_score is not None else "未打分"
        )
        ...
        if failed_cases:
            ...
            for code, case_ids in code_map.items():
                lines.append(
                    f"- **报错代号 {code}**: 用例编号涵盖 {', '.join(case_ids)}"
                )
        else:
